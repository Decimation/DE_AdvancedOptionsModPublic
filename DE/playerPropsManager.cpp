...
idDeclInventory* PlayerPropsManager::getEquipmentDecl(equipmentType_t equipmentType)
{
	...
	if (playerPtr) {
		...
		idDeclPlayerProps_validEquipmentEntry_t* equipmentEntrPtr = (idDeclPlayerProps_validEquipmentEntry_t*)
			validEquipmentList->list;
...
std::string PlayerPropsManager::getDebugStrV2()
{
	...
	if (playerPtr) {
		...
		logInfo("getDebugStrV2: validEquipmentList->list: %p, validEquipmentList->num: %d", validEquipmentList->list,
		        validEquipmentList->num);
